#!/usr/bin/env node
// Seeds LibriTTS-R harvested voices into R2 + database
import fs from "fs";
...
const VOICES_DIR = path.join(__dirname, "system-voices-libritts");
...
async function main() {
  ...
  console.log(`Seeding ${names.length} LibriTTS voices...`);
  ...
  for (const name of names) {
    ...
    const category = CATEGORY_MAP[info.category] || "PODCAST";
